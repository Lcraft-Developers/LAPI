...
public abstract class Inventory implements Listener {
	...
	public Inventory(SpigotClass plugin, Inventory backInv, boolean canBack, String id, LanguagesManager languagesManager) throws IOException {
	...
	public org.bukkit.inventory.Inventory getListPage(ArrayList<ItemStack> items, int page, int invSize, UUID player, String title, Material placeHolder) throws IOException {
		...
		BACK_TO_LAST_INV = new ItemBuilder(Material.ORANGE_BANNER).setDisplayName(getLanguagesManager().getIDLanguage(getLanguagesManager().getIDFromUUID(player)).translate("§aGo back to last Inv"));
		BACK_TO_PAGE = new ItemBuilder(Material.RED_BANNER).setDisplayName(getLanguagesManager().getIDLanguage(getLanguagesManager().getIDFromUUID(player)).translate("§aGo back to last Page"));
		NEXT_TO_PAGE = new ItemBuilder(Material.LIME_BANNER).setDisplayName(getLanguagesManager().getIDLanguage(getLanguagesManager().getIDFromUUID(player)).translate("§aGo next Page"));
	...
	public String getListTitle(String title, UUID player, int page, int pages) throws IOException {
		title = getLanguagesManager().getIDLanguage(getLanguagesManager().getIDFromUUID(player)).translate(title + " §6%cpage%§7/§6%maxpage%").replace("%cpage%", page + "").replace("%maxpage%", pages + "");
		return title;
	}
