...
public class PermsManager {
    ...
    public boolean hasPermissions(LPlayer p, String permission) {
        ...
        if (ProxyServer.getInstance().getPluginManager().getPlugin("LuckPerms") != null && isLuckPermsEnabled()) {
            LuckPerms api = (LuckPerms) ProxyServer.getInstance().getPluginManager().getPlugin("LuckPerms");
    ...
    public class CustomCalculator implements ContextCalculator<ProxiedPlayer> {
        ...
        @Override
        public void calculate(ProxiedPlayer player, ContextConsumer contextConsumer) {
            for(String c : next) {
